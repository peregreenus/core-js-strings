 ...
 */
function remover(str, value, idx) {
  return idx >= 0
...
function removeFirstOccurrences(str, value) {
  ...
  return remover(str, value, startIdx);
}
...
function sumOfCodes(str) {
  try {
    return [...str].reduce((sum, _, idx) => sum + str.charCodeAt(idx), 0);
  } catch (e) {
    return 0;
  }
}
...
function countVowels(str) {
  let length;
  try {
    length = str.match(/[aeiouy]/gi).length;
  } catch (e) {
    length = 0;
  }
...
function isPalindrome(str) {
  const string = str.toLowerCase().replace(/[^a-zA-Z]/g, '');
...
function encodeToRot13(str) {
  const regExp1 = /[A-Ma-m]/;
  const regExp2 = /[N-Zn-z]/;
  return [...str]
    .map((item, idx) => {
      if (regExp1.test(item)) {
        return String.fromCharCode(str.charCodeAt(idx) + 13);
      }
      if (regExp2.test(item)) {
        return String.fromCharCode(str.charCodeAt(idx) - 13);
      }
      return item;
    })
